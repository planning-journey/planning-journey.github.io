    ...
    >
      ...
      >
        <div className="flex justify-between items-start mb-6">
          <h2 className="text-2xl font-bold text-white">목표 관리</h2>
            ...
            {goals && goals.length > 0 ? (
                <div className="flex flex-col gap-3">
                    {goals.map((goal) => (
                        <div 
                          key={goal.id} 
                          className="flex items-center p-4 bg-slate-700 rounded-lg text-slate-300 border border-slate-600 shadow-sm"
                        >
                          <Flag style={{ color: goal.color }} className="w-5 h-5 mr-3" />
                          <span className="font-medium text-white">{goal.name}</span>
                          <span className="ml-auto text-sm text-slate-400">{goal.periodType === 'daily' ? '매일' : goal.periodType === 'weekly' ? '매주' : '매월'}</span>
                        </div>
                    ))}
                </div>
            ) : (
                <p className="text-slate-400 text-center py-10">아직 목표가 없습니다. 새로운 목표를 추가해보세요!</p>
            )}
        </div>
